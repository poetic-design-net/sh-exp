...
export async function getWooCommercePaymentInfo(subscription) {
    try {
        let paymentInfo = {
            price: 0,
            currency: 'EUR',
        ...
        if (subscription.wooCommerceSubscriptionId) {
            const { data: wooSubscription } = await api.get(`subscriptions/${subscription.wooCommerceSubscriptionId}`);
            paymentInfo = {
                ...paymentInfo,
            ...
            const { data: subscriptionMeta } = await api.get(`subscriptions/${subscription.wooCommerceSubscriptionId}/meta_data`);
            const stripeMeta = subscriptionMeta.find((meta) => meta.key === '_stripe_subscription_id');
            if (stripeMeta) {
        ...
        if (subscription.wooCommerceOrderId) {
            const { data: order } = await api.get(`orders/${subscription.wooCommerceOrderId}`);
            paymentInfo = {
                ...paymentInfo,
...
export async function syncWooCommercePaymentInfo(subscription) {
    try {
