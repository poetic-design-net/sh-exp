...
export function useUserProfile() {
    ...
    const subscriptionsRef = useMemo(() => {
        if (!user?.uid)
            return null;
        return query(collection(firestore, 'subscriptions'), where('userId', '==', user.uid), where('status', '==', 'active'));
    }, [user?.uid, firestore]);
    ...
    const { data: profile, status: profileStatus } = useFirestoreDocData((userRef || dummyDocRef), { idField: 'id', suspense: false });
    const { data: subscriptions = [], status: subscriptionsStatus } = useFirestoreCollectionData((subscriptionsRef || dummyCollectionRef), { idField: 'id', suspense: false });
    ...
    const activeSubscriptions = useMemo(() => {
        if (!subscriptionsRef || !subscriptions)
            return [];
        const now = Timestamp.now().toMillis();
        return subscriptions.filter(sub => sub.status === 'active' && sub.endDate > now);
    }, [subscriptionsRef, subscriptions]);
    // Only return real data when we have valid references
    return {
        profile: userRef ? profile : undefined,
        activeSubscriptions,
        isLoading
    };
}
...
export function useHasMembership(membershipId) {
