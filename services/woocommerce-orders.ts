...
export async function getWooOrders(page: number = 1, perPage: number = 10) {
  try {
...
export async function getWooOrderById(orderId: number) {
  try {
...
export async function getWooOrdersByCustomerId(customerId: number) {
  try {
...
export async function getWooOrdersBySubscriptionId(subscriptionId: number) {
  try {
    const { data } = await api.get('orders', {
...
export function getStripeInfoFromOrder(order: WooOrder) {
  const stripeInfo = {
    paymentIntentId: '',
    subscriptionId: '',
    customerId: ''
...
export async function syncOrderWithFirebase(order: WooOrder) {
  try {
