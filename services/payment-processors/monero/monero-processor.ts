...
interface MoneroPaymentDetails {
  address: string;
  amount: number;
  paymentId: string;
}

export class MoneroProcessor implements PaymentProcessor {
  private moneroAddress: string;
  private secretViewKey: string;
  private wallet: any; // MoneroWallet type
  ...
  private getMoneroAmount(eurAmount: number): number {
  ...
  async createCheckoutSession(options: PaymentOptions): Promise<PaymentSession> {
    ...
    const paymentDetails: MoneroPaymentDetails = {
